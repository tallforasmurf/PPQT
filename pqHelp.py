...
The source code of PPQT is licensed for noncommercial use under the GPL.</p>
<h3>Acknowledgements</h3>
<p>First a deep bow of respect to Steve Shulz (Thundergnat) who created and maintained Guiguts, the program from which PPQT takes inspiration and lots of ideas. Second, a genuflection  to Mark Summerfield for the book <i>Rapid GUI Developement with PyQt</i> which made building a large application easier.</p>
...
<p>Use the menu command File > Check for Updates. PPQT queries github, where
its source is maintained, to see if any of the program modules have been updated. If so, it displays a count of the changed modules and asks if you
want to download them. If you approve, it downloads the altered source modules
and installs them. The next time you start PPQT the updated code is in effect.</p>
<p>The update process checks for a number of possible errors and exits safely under most conditions. However, it is remotely possible that an update could end partially incomplete, with the PPQT code in an inconsistent state. In this case a message tells you to download a complete new PPQT distribution. (Alternatively, restore the complete PPQT folder from a backup source.)</p>
<p>At this time automatic update only operates on Python source modules and
the Help html text. It does not update dictionary files or the extras folder.</p>
...
on input and back to some byte code on output. The conversion is based on the file suffix. Rename a new file as needed to get these encodings:</p>
<table border='1'>
...
<p>PPQT will save to any of the above encodings based on the file suffix of the output file. However <tt>.utf</tt> is preferred if the text contains non-Latin-1 characters and <tt>.txt</tt> when you are sure it is Latin-1 or strict ASCII.</p>
<h3>Metadata</h3>
When opening a book, PPQT looks for a file of metadata in the same location. The metadata file has the full book name plus <tt>.meta</tt>, for example <tt>afinebook.txt.meta</tt> If this meta file
is not found, PPQT assumes this is the first time it has opened this book.
It scans the book text gathering information including the location of page separator lines. These metadata will be stored in the <tt>.meta</tt> file when the file is saved. When the <tt>.meta</tt> file does
exist, PPQT loads metadata information from it.</p>
...
the vertical "splitter" between the halves left and right. These adjustments are remembered from session to session.</p>
...
<h3>Edit Keys</h3>
<p>The following key commands are available when keyboard focus is in the Edit panel.</p>
<ul>
...
<li>Add shift key to any of the above: extend selection</li>
...
<h3>Spellcheck</h3>
<h4>Good and Bad Word Files</h4>
<p>The first time PPQT opens a book (i.e. when no metadata file exists), it
looks for a file <tt>good_words.txt</tt> (or <tt>.utf</tt>) and loads its contents. It also looks for a file <tt>bad_words.txt</tt> (or <tt>.utf</tt>). The "good" and "bad" words are saved in the metadata file thereafter. More "good" words can be added from the Words panel.</p>
<h4>Spellcheck Operation</h4>
<p>When building or refreshing its metadata, PPQT checks all words for spelling. A word in the "bad" list is assumed to be misspelt; one in the "good" list is assumed to be correct. A word not in those lists is presented to the spell-checker and noted as correct or misspelt based on the current dictionary.</p>
<h4>Displaying Misspelt Words</h4>
<p>Select View&nbsp;>&nbsp;Spelling to turn on or off red-underlining of misspelt words. Turning it on may take some seconds in a large book.</p>
<h4>Dictionaries</h4>
Use View&nbsp;>&nbsp;Dictionary to select a dictionary for the main language of the book. Use the Refresh button of the Words panel to spellcheck with the new dictionary. Dictionaries for several languages are included. To add another dictionary, place Myspell/OpenOffice compatible <tt>.aff</tt> and <tt>.dic</tt> dictionary files in the <tt>dict</tt> folder in the PPQT folder.</p>
<p>You can mark any word, phrase or section for
spell-check with an alternate dictionary with the nonstandard tag <b><tt>&lt;sd</tt></b>&nbsp;<i>dictag</i><b>&gt;</b>...<b><tt>&lt;/sd></tt></b>, for example<br />
<tt>He whispered &lt;sd fr_FR>&lt;i>Je t'aime&lt;/i>&lt;/sd></tt>.<br />
Using alternate dictionary tags can make spellcheck take significantly longer.</tt>
<h2>Pngs Panel</h2>
...
in the metadata file and reloaded with the book.</p>
<p>When the focus is in the Notes panel, the alt-ctl-L key causes the
current line number of the Edit panel cursor position to be entered, in curly braces: <tt>{1475}</tt>. (Note: under Ubuntu, alt-ctl-L may be the keyboard shortcut for Lock Screen. Use Ubuntu's Preferences &gt; Keyboard Shortcuts to change this to some other combination.)</p>
<p>The alt-ctl-P key causes the current book page
number to be entered in square brackets: <tt>[214]</tt>. Use these keys
to relate your notes to locations in the book, for example <tt>oe lig on {1475}; big table starts on [214]</tt>.
</p><p>
...
<table border='1'>
<tr><td style='width:4em;'>Next</td><td>Search for the Find text beginning at the edit cursor and going toward the bottom boundary. Pressing Return in the Find text field, or keying ctl-g in the Edit panel, is the same as clicking Next.</td></tr>
<tr><td>Prior</td><td>Search for the Find text beginning at the edit cursor and going back toward the top boundary. Keying ctl-shift-g in the Edit panel is the same as clicking Prior.</td></tr>
...
have the option of cancelling the operation. This replace-all 
...
<p>To save the button settings to a file, select File>Save Find Buttons. Provide the name and location for the saved file, which will be saved with UTF-8 encoding (so a suffix of .utf is a good idea).</p>
...
<tr><td><b>/R..R/</b></td><td>Each line is aligned to the current right margin.</td></tr>
...
<h4>Indents</h4>
<p>The top row of controls set the default indents for block quotes (/Q..Q/).
...
<p>The second row of controls set the indents for poetry, in which the <i>First</i> indent applies to every line.
The <i>Left</i> indent is used only if a line
...
specify <tt>D:'<i>x</i>'</tt> to set a different decimal separator character <i>x</i>, for example in some European texts the comma is the conventional
...
<tr><td><b>X/</b>, <b>C/</b>, <b>*/</b></td><td><tt>&lt;/pre></tt></td></tr>
<tr><td><b>/T</b>, <b>/TM</b></td><td><tt>&lt;table></tt> or <tt>&lt;table style="width:<i>pp</i>%;"></tt> where <i>pp</i> is the specified table width as a percent of 75.</tt></td></tr>
...
<i>pp</i> is the specified width as a percent of the table width.
...
Put the normal PG <tt>images</tt> folder at the same place as the book text
and images should be found.</p>
<p>When keyboard focus is in the Preview Panel, keying ctl-f opens a 
minimal Find dialog so you can search for text in the displayed page.
The selected text is preloaded in the dialog. The dialog goes forward and
wraps around at the end of the page.</p>
<p>You can click a link in the Preview panel and the linked page or image
will load. There is no Back button as on a normal browser. However,
ctl-left-arrow should perform the Back function. Otherwise, click Refresh
to redisplay the book text.</p>
