...
a plum-colored background on any word that appears in the scannos file.</p>
<h3>Spellcheck</h3>
<p>PPQT looks for a file <tt>good_words.txt</tt> (or <tt>.utf</tt>) when it opens the text for the first time, and loads its contents. It looks for a file <tt>bad_words.txt</tt> (or <tt>.utf</tt>) also. These "good" and "bad" words are saved in the metadata file thereafter. More "good" words can be added from the Words panel.</p>
<p>When (re)building its metadata, PPQT checks all words for spelling. It tries to attach Aspell (version 0.60.x, the latest) as a subprocess. A "bad" word is assumed to be misspelt; a "good" word is assumed to be correct. Any word not in those lists is presented to Aspell (if it is attached), and noted as correct or misspelt.</p>
<p>Use View > Spelling to turn on red-underlining of misspelt words.</p>
<h2>Pngs Panel</h2>
...
the Find panel.</p>
<p>TBS: a way to add a word to good-words and clear its X.
TBS: a way to filter the table down to just first-harmonic matches
to a word. TBS: detection of "suspects" that match except for case, hyphens or apostrophes.</p>
